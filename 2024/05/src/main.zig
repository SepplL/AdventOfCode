...
pub fn main() !void {
    ...
    const result1 = try parse_printing_rules(buffer, false);
    print("Result for sum after applying part 1 printing rules: {d}\n", .{result1});

    const result2 = try parse_printing_rules_part2(buffer, false);
    print("Result for sum after applying part 2 printing rules: {d}", .{result2});
}
...
fn parse_printing_rules_part2(input: []const u8, verbose: bool) !usize {
    var result: usize = 0;
    const max_size: usize = 100;
...
test "day 05 part 02" {
    ...
    const result = try parse_printing_rules_part2(buffer, true);
    print("Calculated Sum {d} in test input. \n", .{result});
